...
impl FromStr for Person {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        // Step 2: Check if the split operation returns exactly 2 elements
        if parts.len() != 2 {
            return Err(ParsePersonError::BadLen);
        }

        let name = parts[0].trim().to_string();
        // Step 3: If the name is empty, return NoName error
        if name.is_empty() {
            return Err(ParsePersonError::NoName);
        }

        // Step 5: Parse the second element (age) into u8
